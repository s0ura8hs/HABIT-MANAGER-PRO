...
class HabitManager {
    constructor() {
        ...
        // Initialize alarm sound functionality
    ...
    // Initialize alarm sound using Web Audio API
    ...
    playAlarmSound(duration = 2000) {
        ...
        try {
            ...
            // Create a pleasant alarm sound with multiple tones
            ...
            console.warn('Error playing alarm sound:', e);
        }
    }

    // Setup beforeunload event to prompt data download
    setupBeforeUnloadWarning() {
        window.addEventListener('beforeunload', (e) => {
            // Check if there's any data to save
            const hasData = this.habits.length > 0 || this.goals.length > 0 || this.tasks.length > 0;
            ...
            if (hasData) {
                ...
                const message = 'You have unsaved data. Would you like to download a backup before leaving?';
                
                // Try to trigger download (browsers may block this)
                setTimeout(() => {
                    if (confirm('Would you like to download your data before leaving?')) {
                        this.exportData();
                    }
                }, 100);
    ...
    pomodoroCompleted() {
        ...
        if (!this.pomodoroTimer.isBreak) {
            // Work session completed - starting break
            this.playAlarmSound(2000); // Play alarm for 2 seconds
    ...
    resetData() {
        if (confirm('Are you sure you want to reset all data? This cannot be undone.')) {
            ...
            localStorage.removeItem('habitManager_moods');
